  ...
  async function handleCheckout() {
    ...
    try {
      ...
      const body = {
        amount,
        currency,
        channel_id: '0d780246-1152-427f-931d-48f10729e748',
        order: {
        ...
        customer: {
          id: 'checkout-ghada',
          email: 'g.tarek@lite.sa',
          first_name: 'Ghada',
          last_name: 'Tarek',
          phone_country_code: '+20',
          phone_number: '1063961597',
          billing_address: {
            line1: '123 High St.',
            line2: 'Apartment 5B',
            city: 'Riyadh',
      ...
      // The Lite SDK secret is a JWT. The sessions API returns it in the
      // hosted_url fragment (…/pay/<id>#<jwt>); older shapes returned it as a
      // top-level field, so fall back to those too.
      const hostedJwt = data.hosted_url?.includes('#')
        ? data.hosted_url.split('#')[1]
        : '';
      const clientSecret =
        data.client_secret || data.clientSecret || data.token || hostedJwt;
